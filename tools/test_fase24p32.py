...
- Tras crear installed/ con installer.elf, el explorer lista la raiz
  con columnas Nombre/Tam ("exp: items=52 cols=2") y screendump con
...
if not waitstr('exp: items=52 cols=2', 10):
    print('FAIL - items'); print(''.join(acc)[-500:]); qemu.terminate(); sys.exit(1)
check('explorer lista la raiz (items=52, 2 columnas)', True)
time.sleep(0.5)
