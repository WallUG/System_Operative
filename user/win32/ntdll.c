...
void RtlZeroMemory(void *p, uint32_t n)
{
    uint8_t *b = (uint8_t *)p;
    while (n--)
        *b++ = 0;
...
void RtlCopyMemory(void *dst, const void *src, uint32_t n)
{
...
uint32_t RtlWow64GetProcessMachines(void)
{
    return 0;
}

/* GetModuleHandle: devuelve la base del modulo llamador (0 = self). */
uint32_t GetModuleHandle(const char *name)
{
    (void)name;
