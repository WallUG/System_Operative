...
| 25   | **Fase 25 (W2A): set de inicialización W de MSVC — reloj real**: primer paso hacia .exe reales compilados por Microsoft (MSVC, APIs Unicode W). **Reloj REAL** (los stubs devolvían 0): nuevo syscall `SYS_QPC 45` que devuelve `edx:eax` = contador de alta resolución del PIT (latch + `tick_count*divisor + (divisor-cnt)`, serie continua entre ticks); `GetSystemTimeAsFileTime` = FILETIME real con origen fijo 2024-01-01 (todo aritmética 32-bit, sin `__udivdi3` en ring 3), `QueryPerformanceCounter/Frequency` reales (frec 1193182), `GetTickCount` = ticks×10 ms, y **`Sleep` real** (espera por ticks, antes busy-wait de 100k iteraciones). **Versiones W**: `GetCommandLineW` (TIB ASCII → UTF-16 en buffer estático por proceso), `GetEnvironmentStringsW` (+Free), `GetModuleFileNameW`, `GetCurrentThreadId` (= pid de la tarea, cada hilo del kernel tiene el suyo). Ya existían: secciones críticas, VirtualProtect (no-op), VirtualAlloc (heap). Test `tools/test_fase25w2a1.py` **9/9 PASS** (cmdlineW/moduleW/envW/tid/ticks reales/qpc reales/filetime ≥ 2024 y creciente/cs/exit 0). Regresión completa PASS (20a-d, faseE, fase22, fase23 completo, fase24 completo) — ajustado `test_fase24p32.py` (items=51, la raíz ganó w2atest.exe). Próximo: thunks W→A masivos de kernel32 (~50) y el primer binario MSVC. |
